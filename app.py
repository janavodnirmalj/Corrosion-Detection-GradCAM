...
from HRNET_do import HRNet_dropout
...
def corrosion_detection(files):
    ...
    for file in files:
        ...
        if cor_class:
            all_classes.append(cor_class)
        if mean_score:
            all_percentages.append(f"{mean_score}% ± {round(upper_score - mean_score, 2)}%")
        yield (gr.update(value=all_cam_images), gr.update(value="\n".join(all_classes)), gr.update(value="\n".join(all_percentages)), gr.update(value=all_overlay_images))
...
with gr.Blocks() as iface:
    ...
    gr.Markdown("### Detection Results (Streaming):")

    output_gallery = gr.Gallery(label="Grad-CAM Heatmap", columns=5)
    corrosion_classes = gr.Textbox(label="Corrosion Classes")
    corroded_percentages = gr.Textbox(label="Corroded Percentages")
    overlay_gallery = gr.Gallery(label="Corrosion Overlay", columns=5)

    process_button.click(corrosion_detection, inputs=image_input, 
                             outputs=[output_gallery, corrosion_classes, corroded_percentages, overlay_gallery])
